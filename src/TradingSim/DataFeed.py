...
class DataProcessor:
    ...
    #TODO: Re-do transformations and calcs as numpy operations using cython
    """
    def calculate_indicators_np(self, **data, indicators):

        # Provide base data to indicator class
        Ind = Indicator(
            open_arr=data['open'],
            high_arr=data['high'],
            low_arr=data['low'],
        ...
        # Add keys for each indicator we want
        for ind_name in indicators:
            ind_lower = ind_name.lower()
            calc_ind = getattr(ind_lower, Ind)
            data[ind_lower] = calc_ind()
    """
...
class StockData:
    ...
    def select_from_preprocessed(self, env_num):
        try:
            # Prepare the query
            query = sql.SQL("""
              SELECT * 
              FROM sharadar.preprocessed p
              WHERE p.env_num = {env_num};
              """).format(
                env_num=sql.Literal(env_num)
            )
...
class DataHelper:
    ...
    def dataset_worker(self, worker_num):
        ...
        while True:
            ...
            if db.select_from_preprocessed(worker_num):
                ...
                while row_data:
                    data = {col: row_data[col] for col in self.col_list}
                    ticker_data[row_data['ticker']] = data
                    ticker_cnt += 1
                    if ticker_cnt >= self.num_assets:
                        self.output_queues[worker_num].put(ticker_data)
                    row_data = db.cur.fetchone()
    ...
    def start(self):
        ...
        for i in range(self.num_envs):
            p = multiprocessing.Process(
                target=self.preprocessing_worker if self.mode == 'process' else self.dataset_worker, args=(i,)
            )
            processes.append(p)
            p.start()
